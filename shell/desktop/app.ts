...
export function applySurfaceActionEvent(state: DesktopShellState, event: BusEnvelope): void {
    ...
    if (event.topic === "shell.action.completed" && body.decision !== "denied") {
        ...
        if (body.action === "surface.always_on_top") {
            state.surfaces = state.surfaces.map((entry) => entry.id === body.surface_id ? { ...entry, always_on_top: body.result_state?.always_on_top ?? body.always_on_top ?? entry.always_on_top, action_error: undefined, disabled: false } : entry);
            mergeActionReadback(state, body);
            return;
        }
        if (body.action === "surface.fullscreen") {
            state.surfaces = state.surfaces.map((entry) => entry.id === body.surface_id ? { ...entry, fullscreen: body.result_state?.fullscreen ?? body.fullscreen ?? entry.fullscreen, action_error: undefined, disabled: false } : entry);
            mergeActionReadback(state, body);
            return;
        }
        ...
        if (body.action === "surface.raise") {
            mergeActionReadback(state, body);
            const stack = body.result_state?.stack;
            if (stack) {
                state.surfaces = state.surfaces.map((entry) => entry.id === body.surface_id ? { ...entry, ...stack, action_error: undefined, disabled: false } : entry);
            }
            return;
        }
    }
    if (body.action !== "surface.focus" && body.action !== "surface.raise" && body.action !== "surface.close" && body.action !== "surface.move" && body.action !== "surface.resize" && body.action !== "surface.tile" && body.action !== "surface.always_on_top" && body.action !== "surface.fullscreen" && body.action !== "surface.maximize") {
        return;
    }
