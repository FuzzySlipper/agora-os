...
fn relocate_maps<'a, I: Iterator<Item = &'a Relocation>>(
    ...
    for (rel_n, rel) in relocations.enumerate() {
        ...
        // Handle BPF helper call relocations (LLVM >= 16 codegen).
        // LLVM 16+ generates helper calls as PSEUDO_CALL with imm=-1 and
        // R_BPF_64_32 relocation to external (UNDEF) helper symbols.
        // Resolve the helper number from a built-in name-to-ID mapping.
        if sym.section_index.is_none() && rel.size == 32 {
            if let Some(helper_id) = bpf_helper_id_from_name(sym.name.as_deref()) {
                let ins = &mut instructions[ins_index];
                ins.imm = helper_id as i32;
                ins.set_src_reg(0); // Clear PSEUDO_CALL bit -> normal helper call
            }
            continue;
        }
...
impl<'a> FunctionLinker<'a> {
    ...
    fn relocate(&mut self, program: &mut Function, fun: &Function) -> Result<(), RelocationError> {
        ...
        for ins_index in start_ins..start_ins + n_instructions {
            ...
            // BPF helper calls (LLVM >= 16 codegen) have PSEUDO_CALL bit set
            // and imm=-1 before relocation. After relocate_maps patches them,
            // imm is the helper number and src_reg is 0, so insn_is_call returns
            // false and the above check handles it. Handle stray unpatched calls:
            if is_call && rel.is_none() && ins.imm == -1 {
                debug!("skipping unresolved helper call at ins {ins_index} in {}", fun.name);
                continue;
            }
...
/// Map a BPF helper function name (extracted from a Rust-mangled symbol) to
/// the standard kernel helper function ID.
///
...
fn bpf_helper_id_from_name(name: Option<&str>) -> Option<u32> {
    ...
    // length. Search for common BPF helper function names as suffix.
    //
    // Standard Linux BPF helper IDs:
    // https://elixir.bootlin.com/linux/latest/source/include/uapi/linux/bpf.h
    if name.contains("bpf_map_lookup_elem") { Some(1) }
    else if name.contains("bpf_map_update_elem") { Some(2) }
    else if name.contains("bpf_map_delete_elem") { Some(3) }
    else if name.contains("bpf_probe_read") && !name.contains("_str") && !name.contains("_user") && !name.contains("_kernel") { Some(4) }
    else if name.contains("bpf_ktime_get_ns") { Some(5) }
    else if name.contains("bpf_get_prandom_u32") { Some(7) }
    else if name.contains("bpf_get_smp_processor_id") { Some(8) }
    else if name.contains("bpf_tail_call") { Some(12) }
    else if name.contains("bpf_get_current_pid_tgid") { Some(14) }
    else if name.contains("bpf_get_current_uid_gid") { Some(15) }
    else if name.contains("bpf_get_current_comm") { Some(16) }
    else if name.contains("bpf_get_cgroup_classid") { Some(17) }
    else if name.contains("bpf_perf_event_output") { Some(25) }
    else if name.contains("bpf_get_stackid") { Some(27) }
    else if name.contains("bpf_get_current_task") { Some(35) }
    else if name.contains("bpf_probe_read_str") && !name.contains("_user") && !name.contains("_kernel") { Some(45) }
    else if name.contains("bpf_get_socket_uid") { Some(47) }
    else if name.contains("bpf_get_current_cgroup_id") { Some(80) }
    else if name.contains("bpf_get_local_storage") { Some(81) }
    else if name.contains("bpf_probe_read_user") && !name.contains("_str") { Some(112) }
    else if name.contains("bpf_probe_read_kernel") && !name.contains("_str") { Some(113) }
    else if name.contains("bpf_probe_read_user_str") { Some(114) }
    else if name.contains("bpf_probe_read_kernel_str") { Some(115) }
    else if name.contains("bpf_ringbuf_output") { Some(130) }
    else if name.contains("bpf_ringbuf_reserve") { Some(131) }
    else if name.contains("bpf_ringbuf_submit") { Some(132) }
